"""
A worker handles the connection to InfluxDB.
"""
import signal
import logging
import time
...
class Worker(object):
    """
    Implementation of worker class that handles InfluxDB
    ...
    def consume(self):
        ...
        logging.info("Listening for messages on Lanz...")
        ...
        while not killer.kill_now:
            try:
                ...
                logging.debug("Sleeping for %d ms before reconnect",
                              self.config.lanz_reconnect_wait_time_ms)
                time.sleep(self.config.lanz_reconnect_wait_time_ms / 1000.0)
